...
public class MaterialController {
    ...
    public ResponseEntity<MaterialResponse> generateMaterial(@RequestBody MaterialGenerationRequest request, Authentication authentication) {
        String username = authentication.getName();
        User user = userService.findByUsername(username).orElseThrow(() -> new RuntimeException("User not found"));

        Material material = materialService.generateMaterial(request.getType(), request.getGrade(), request.getSubject(), request.getTopic(), user);
